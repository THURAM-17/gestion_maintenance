<?php
// Page de modification d'une planification de maintenance
...
// Récupérer les informations de la planification
...
$success = 'Planification mise à jour avec succès';
...
<h1 class="text-2xl font-bold text-gray-800">Modifier une planification de maintenance</h1>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="md:col-span-2">
        ...
        <textarea id="description" name="description" rows="3"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"><?php echo htmlspecialchars($schedule['description']); ?></textarea>
    </div>
</div>
</div>
...
<div id="scheduleItems" class="space-y-6">
    <!-- Les éléments existants seront ajoutés dynamiquement -->
</div>

<div id="noItemsMessage" class="p-4 text-center text-gray-500" style="<?php echo count($schedule_items) > 0 ? 'display: none;' : ''; ?>">
...
// Si un élément existant est fourni, pré-remplir les champs
if (existingItem) {
// Sélectionner l'équipement
const equipmentSelect = newItem.querySelector('[name*="equipment_id"]');
const options = equipmentSelect.options;
for (let i = 0; i < options.length; i++) {
    if (options[i].value == existingItem.equipment_id) {
...
const taskSelect = newItem.querySelector('[name*="task_id"]');
const taskOptions = taskSelect.options;
for (let i = 0; i < taskOptions.length; i++) {
    if (taskOptions[i].value == existingItem.task_id) {
...
if (existingItem.planned_date) {
    newItem.querySelector('[name*="planned_date"]').value = existingItem.planned_date;
}

// Sélectionner le mois
if (existingItem.planned_month) {
    const monthSelect = newItem.querySelector('[name*="planned_month"]');
    const monthOptions = monthSelect.options;
    for (let i = 0; i < monthOptions.length; i++) {
        if (monthOptions[i].value == existingItem.planned_month) {
...
if (existingItem.assigned_to) {
    const assignedSelect = newItem.querySelector('[name*="assigned_to"]');
    const assignedOptions = assignedSelect.options;
    for (let i = 0; i < assignedOptions.length; i++) {
        if (assignedOptions[i].value == existingItem.assigned_to) {
...
const statusSelect = newItem.querySelector('[name*="status"]');
const statusOptions = statusSelect.options;
for (let i = 0; i < statusOptions.length; i++) {
    if (statusOptions[i].value == existingItem.status) {
...
if (existingItem.notes) {
    newItem.querySelector('[name*="notes"]').value = existingItem.notes;
}
}
...
const existingItems = <?php echo json_encode($schedule_items); ?>;
